...
class Embedding(TokenEmbedder):
    ...
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'Embedding':
        ...
        embedding_dims = params.pop('embedding_dim')
        pretrained_files = params.pop("pretrained_file", None)
        ...
        if pretrained_files:
            ...
            for i in range(len(embedding_dims)):
                if pretrained_files[i]:
                    embeddings.append(_read_pretrained_embeddings_file(pretrained_files[i], embedding_dims[i],vocab,vocab_namespace))
                else:
                    embeddings.append(torch.zeros(embedding_dims[i]))
                embedding_dim+=embedding_dims[i]
            weight = torch.cat(embeddings, -1)
