...
class CoreferenceResolver(Model):
    ...
    context_layer_back: ``Seq2SeqEncoder``, optional (default=``None``)
        If provided this layer incorporates contextual information for each word in the
        document from the backward pass and concatenates it with the context_layer outputs.
    ...
    def forward(self,  # type: ignore
        ...
        if self._context_layer_back is not None:
            contextualized_embeddings_back = self._context_layer_back(text_embeddings, text_mask)
            contextualized_embeddings = torch.cat([contextualized_embeddings,contextualized_embeddings_back], -1)
        ...
        meta_emb = self._embed_genre_and_speaker(metadata, top_spans.split(1, dim=-1)[0], valid_antecedent_indices)
    ...
    def _embed_genre_and_speaker(self,
                                 ...
                                 antecedents):
        '''
        Computes the speaker and genre embeddings
        :param metadata:
        :param top_span_starts:
        :param antecedents:
        :return: the embedding of the speaker and genre features
        '''
        device = self._get_prediction_device()
        if device==-1:
            import torch as tch
        else:
            import torch.cuda as tch
        feature_emb_list = []
        if self._speaker_embedding is not None:
            speaker_ids = tch.LongTensor(metadata[0]["speaker_ids"])
            mention_speaker_ids = torch.index_select(speaker_ids, 0, top_span_starts.view(-1).type(tch.LongTensor))
            antecedent_speaker_ids = torch.index_select(mention_speaker_ids, 0, antecedents.view(-1).type(
                tch.LongTensor)).view(antecedents.shape)
            same_speaker = torch.unsqueeze(mention_speaker_ids, 1) == antecedent_speaker_ids
            speaker_pair_emb = self._speaker_embedding(same_speaker.type(tch.LongTensor))
            feature_emb_list.append(speaker_pair_emb)

        if self._genre_embedding is not None:
            genre = tch.LongTensor([metadata[0]["genre"]])
            genre_emb = self._genre_embedding(genre).repeat(antecedents.unsqueeze(-1).shape)
            feature_emb_list.append(genre_emb)

        feature_emb = torch.cat(feature_emb_list, -1)
        feature_emb = self._feature_dropout(feature_emb)

        return feature_emb.unsqueeze(0)
